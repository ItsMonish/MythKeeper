...
def getUserSharing(username: str) -> list:
    ...
    records = dbCursor.execute(
        "SELECT * FROM clientShares WHERE username=?", (username,)
    ).fetchall()
    if len(records) == 0:
        return shareList
    for rec in records:
...
def loseSharing(resHash: str, username: str, share: str) -> None:
    ...
    dbCursor.execute(
        "DELETE FROM clientShares WHERE username=? AND share=?",
        (
            username,
            share,
        ),
    ...
    dbCursor.execute(
        "DELETE FROM serverShares WHERE username=? AND reshash=?", (username, resHash)
    )
    con.commit()
...
def revokeSharing(resource: str, username: str, resHash) -> None:
    con, dbCursor = getDatabaseCursor()
    dbCursor.execute(
        "DELETE FROM clientShares WHERE username=? AND resource=?",
        (
            username,
            resource,
        ),
