...
class utils:
    ...
    def split_ints(self, secret):
        ...
        text = codecs.encode(byte_object, "hex_codec").decode("utf8") + "00" * (
            32 - (len(byte_object) % 32)
        )
    ...
    def merge_ints(self, secrets):
        ...
        try:
            ...
            return (
                codecs.decode(byte_object, "hex_codec").decode("utf8").rstrip("\00\x00")
            )
        except:
    ...
    def to_base64(self, number):
        ...
        result = str(
            base64.urlsafe_b64encode(
                b"\00" * (64 - len(tmp)) + codecs.decode(tmp, "hex_codec")
            ).decode("utf8")
        )
