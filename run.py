...
from app.utils.sssHelper import (
    createShares,
    combineShares,
    storeClientShares,
    storeSeverShares,
    verifySharedSolution,
    getSharedChallenge,
)
...
def serverShares():
    if session.get("username") == None:
        return jsonify({}), 403
    data = request.json
    share, challenge, resHash, username = (
        data["share"],
        data["challenge"],
        data["resHash"],
        data["username"],
...
def revokeShared():
    ...
    if data.get("resHash") != None and data.get("challenge") == "":
        ...
        data.get("resHash") != None
        and data.get("challenge") != ""
        and data.get("share") != None
    ):
